...
class CLKClique : public CLApp {
  int clique_size_;
  std::string file_name_;

 public:
  CLKClique(int argc, char** argv, std::string name, int clique_size, std::string file_name) :
    CLApp(argc, argv, name), clique_size_(clique_size), file_name_(file_name) {
    get_args_ += "c:o:";
    AddHelpLine('c', "", "clique size",
                std::to_string(clique_size_));
    AddHelpLine('o', "ordering", "import ordering generated by external source");
  }

  void HandleArg(signed char opt, char* opt_arg) override {
    switch (opt) {
      case 'c': clique_size_ = atoi(opt_arg);            break;
      case 'o': file_name_ = std::string(opt_arg);            break;
      default: CLApp::HandleArg(opt, opt_arg);
    }
  }
